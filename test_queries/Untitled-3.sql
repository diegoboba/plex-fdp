...
select count(1) from quantio.report_stock;

select top 1000 * from quantio.plex_pedidos p ;

select distinct s.NombreFantasia

from quantio.plex_pedidos p
left join quantio.sucursales s 
    on p.sucursal = s.sucursal;

    select 
        *
    from  quantio.report_stock p 
    where p.IdPedidoGlobal = '40016621';
